...
5. In `utils/makechain.ts` chain change the `QA_PROMPT` for your own usecase. Chanage `modelName` in `new OpenAI` to `gpt-4`, if you have access to `gpt-4` api. Please verify outside this repo that you have access to `gpt-4` api, otherwise the application will not work.
